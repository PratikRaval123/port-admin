...
import { deleteProject, getProjects, reset } from "@/store/features/projectSlice";
...
const ProjectsPage = () => {
    ...
    const { projects, isLoading, isError, message } = useAppSelector(
        (state) => state.project
    );
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col">
                        ...
                        {projects.map((project: Project, key: number) => (
                            <div
                                className={`grid grid-cols-4 border-t border-stroke px-4 py-4.5 dark:border-dark-3 sm:grid-cols-5 md:px-6 2xl:px-7.5 ${key === projects.length - 1 ? "" : "border-b"
                                    }`}
                                key={key}
                            >
                                ...
                                <div className="col-span-1 flex items-center justify-end space-x-3.5">
                                    <Link
                                        href={`/projects/edit/${project._id || project.id}`}
                                    ...
                                    {/* View icon, duplicate of edit for now or just remove */}
                                </div>
                            </div>
                        ))}
                    </div>
